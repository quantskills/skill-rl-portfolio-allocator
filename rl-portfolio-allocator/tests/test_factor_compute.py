...
def test_raw_computation_emits_first_forty_catalog_names(
    price_frame: pd.DataFrame,
) -> None:
    raw = compute_raw_factors(price_frame)
    expected = [spec.name for spec in FACTOR_CATALOG[:40]]

    assert list(raw.columns[-40:]) == expected
...
def test_raw_factor_columns_are_float32_and_warmup_stays_nan(
    ...
    for spec in FACTOR_CATALOG[:40]:
        pre_window = raw[spec.name].iloc[: max(spec.lookback - 1, 1)]
        assert pre_window.isna().all(), (
            f"{spec.name} emitted before its catalog lookback window"
        )
    assert raw.loc[0, factor_names].isna().all()
...
def test_raw_computation_does_not_require_columns_unused_by_first_forty(
    ...
    raw = compute_raw_factors(without_open)
